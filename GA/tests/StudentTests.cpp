...
{
    SECTION("fstream Test")
    {
        Parser p("input/test.txt");
        std::string expectedHeader = ">test header";
        std::string expectSequence = "AGCTAAACTGA";
        bool result = (expectedHeader == p.mHeader) && (p.mSequence == expectSequence);
        REQUIRE(result);
    }
    
    SECTION("Translator Test")
    {
        Translator myTranslator;
        std::string input = "ATGTTTTTC";
        std::string output = myTranslator.StartCodon(input);
        bool result = (output == "MFF");
        REQUIRE(result);
    }
}
